...
\newcommand{\thesistitle}{``stealthy'' ECH} % Your thesis title, this is used in the title and abstract
\newcommand{\degree}{Msc. Computer Science -- Data Science} % Replace with your degree name, this is used in the title page and abstract
\newcommand{\typeofthesis}{dissertation} % dissertation, Final Year Project, report, etc.
\newcommand{\authorname}{Neimhin Christopher Robinson Gunning} % Your name, this is used in the title page and PDF stuff
...
\newcommand{\keywords}{censorship, privacy, security, ECH, steganography} % Replace with keywords for your thesis
\newcommand{\school}{\href{https://scss.tcd.ie}{School of Computer Science and Statistics}} % Your school's name and URL, this is used in the title page
...
\todo{
  \textbf{Please consult with your supervisor on this last item before agreeing, and delete if you do not consent}
}
\vspace{3cm}
...
\chapter*{Abstract}
The TLS protocol does not encrypt the Server Name Indication (SNI) nor the Application Layer Protocol Negotiation (ALPN) lists from IP nodes facilitating a TLS connection.
These data are sensitive and it is desirable in many cases to keep them private. Encrypted Client Hello (ECH) is a proposed solution which encrpyts the entire ClientHello message in which
the SNI and ALPN are contained, however ECH can only be carried out with the agreement of all facilitating nodes. This work proposes TODO how many? a suite of possible designs for a more ``stealthy'' version of
strategy for hiding the sensitive parts of the CLientHello. These strategies should not alert intermediate nodes that the SNI and ALPN list is being kept secret.
Evalutions of the implementation of a subset of these designs are presented.
