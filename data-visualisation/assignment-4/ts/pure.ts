...
export function injury_to_broad_injury(incident: string) {
    switch (incident) {
        case "gunfire":
            return "gunfire";
        case "explosion":
...
export function broad_injury_to_injuries(category: string) {
    switch (category) {
        case "gunfire":
            return ["gunfire"];
        case "explosive":
...
export function flatten_and_count_injuries(bins: Array<any>): Array<any> {
...
export function flatten_and_count_man_woman_child(bins: Array<any>) {
    let counts: { [key: string]: number } = {
        "palestinian man": 0,
        "palestinian woman": 0,
        "palestinian minor": 0,
    ...
    function categorise(d: Fatality) {
        let str;
        ...
        if(d.age < 18) return str + "minor";
        const g = d.gender === "M" ? "man" : "woman";
        return str + g;
    }
    bins.forEach(bin => {
        bin.forEach((item: Fatality) => {
...
export function flatten_and_perpetrator(bins: Array<any>) {
    let counts: { [key: string]: number } = {
        "Israeli civilians": 0,
