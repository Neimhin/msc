...
def runp(alpha=0.5, beta=0.9, beta2=0.9, n=5):
...
plt.suptitle('Gradient Descent with Adam step on $f_T(x)$')
...
T = pd.read_csv("data/T.csv").values
for alpha in [1]:
    for beta in [0.01, 0.9]:
        for beta2 in [0.01, 0.9]:
            alpha_a = alpha # * (1-beta)
            n = 25
            run = runp(n=n, alpha=alpha, beta=beta, beta2=beta2)
            label = f"$\\alpha={alpha_a}$, $\\beta_1={beta}$, $\\beta_2={beta2}$"
