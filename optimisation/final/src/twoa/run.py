...
def grid_search_line_search(alpha=1, n=5):
    T = pd.read_csv("data/T.csv").values
    fg = week6.generate_optimisation_functions(
        T, minibatch_size=n, seed=None)
    o = sgd.StochasticGradientDescent()
...
plt.suptitle('Stochastic Gradient Descent on $f_T(x)$ with mini-batch size of 5, constant and line-search')
...
alpha = 0.001
run = run_constant(alpha, n=5)
label = f"constant $\\alpha={alpha}$"
ax_contour.plot(run["x1"], run["x2"], label=label)
ax_f.plot(run['f'], label=label)
...
alpha = 0.3
run = grid_search_line_search(alpha, n=5)
label = f"line-search"
ax_contour.plot(run["x1"], run["x2"], label=label)
ax_f.plot(run['f'], label=label)
