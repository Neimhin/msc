...
def gradient_descent_constant(step_size=0.0065, start=[0, 0], funcs=f, max_time=0.4):
...
if __name__ == "__main__":
    ...
    for funcs in f, g:
        ...
        for i in range(5):
            ...
            ps = [{"min": 0, "max": 10}, {"min": 0, "max": 18}]
            grs = global_random_search.b_mod(
                costf=funcs["function"], iterations=200, parameters=ps, N=400, M=100, max_time=max_time)
            costs = grs['stats']['it_best_costs']
            ...
            grs = global_random_search.b(
                costf=funcs["function"], iterations=200, parameters=ps, perturb_pc=0.001, N=400, M=100, max_time=max_time)
            costs = grs['stats']['it_best_costs']
        ...
        plt.tight_layout()
        plt.savefig(f"fig/bii-time-{funcs['name']}.pdf")
