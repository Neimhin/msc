...
def f_real(xv):
    return 3 * (xv[0] - 5)**4 + 10 * (xv[1] - 9)**2

f_diff_x = f.diff(x)
f_diff_y = f.diff(y)
def f_grad(xv):
    return np.array([
        f_diff_x.subs(x, xv[0]).subs(y, xv[1]),
        f_diff_y.subs(x, xv[0]).subs(y, xv[1]),
    ])
...
def g_grad(xv):
    return np.array([
        g_diff_x.subs(x, xv[0]).subs(y, xv[1]),
        g_diff_y.subs(x, xv[0]).subs(y, xv[1]),
    ])

def g_real(xv):
    return np.maximum(xv[0] - 5, 0) + 10 * np.abs(xv[1] - 9)
